...
class STNorm(nn.Module):
    ...
    def __init__(self, num_nodes, tnorm_bool, snorm_bool, in_dim, out_dim, channels, kernel_size, blocks, layers):
        ...
        # num = int(self.tnorm_bool) + int(self.snorm_bool)

        self.mlps = nn.ModuleList()
        self.gconv = nn.ModuleList()
        self.cross_product = nn.ModuleList()

        print("--------xxxx--in_dim-----------",in_dim)
        print("--------xxxx--channels-----------",channels)
        print("--------xxxx--num-----------", num)
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs) -> torch.Tensor:
        ...
        """
        print("-------------xxxx-input------------",history_data.shape)
        ...
        for i in range(self.blocks * self.layers):
            ...
            print("------------fxxx--xxxxx---------",x.shape)

            filter = self.filter_convs[i](x)
            b, c, n, t = filter.shape
            filter = torch.tanh(filter)
            print("------------xxxxx--xxxxx---------", x.shape)
            print("------------filter--xxxxx---------",filter.shape)
        ...
        # out = out.squeeze(-1)
        print("---------zuizhongshur--xxxxx-----out-------",out.shape)
        return out
