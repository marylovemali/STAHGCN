...
class FullAttention(nn.Module):
    def __init__(self, mask_flag=True, factor=5, scale=None, attention_dropout=0.1, output_attention=False):
        ...
        # print("---------------FullAttention-进入模块----------")
        self.scale = scale
        # print("---------------self.scale----------",self.scale)
        # print("---------------sqrt(E)---------", self.scale)
        self.mask_flag = mask_flag
        self.output_attention = output_attention
        self.dropout = nn.Dropout(attention_dropout)
        # print("---------------FullAttention--self.scale-----------",self.scale)

    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        # print("---------------FullAttention--进入-----------")
        # print("---------------queries-----------",queries.shape)
        # print("---------------values-----------", values.shape)
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / sqrt(E)
        # print("----------------scale-xxxx------------",scale)
        # blhe, bshe->bhls  blhe,bshe  ->bhle , bhse  ->   bh le x bh es == bhls
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)
        # print("-------scores-xxxx-----", scores.shape)
        # print("-------self.mask_flag------",self.mask_flag)
        if self.mask_flag:
        ...
        # print("------scale * scores ---xxxx-----", (scale * scores).shape)
        # print("------torch.softmax(scale * scores, dim=-1) ---xxxx-----", torch.softmax(scale * scores, dim=-1).shape)
        A = self.dropout(torch.softmax(scale * scores, dim=-1))
        # print("------A ---xxxx-----", A.shape)
        # print("------values ---xxxx-----", values.shape)

        # bhls, bshd-> bh ls   bh sd -> bh ls  *  bh sd -> bhld -> blhd

        V = torch.einsum("bhls,bshd->blhd", A, values)
        # print("------V ---xxxx-----", V.shape)
        #
        # print("---self.output_attention---xxxx-----", self.output_attention)
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        # print("---------显示各参数值----------")
        # print("-------AttentionLayer--attention----------",attention)
        # print("-------AttentionLayer--d_model----------", d_model)
        # print("-------AttentionLayer--n_heads----------", n_heads)
        # print("-------AttentionLayer--d_keys0----------", d_keys)

        d_keys = d_keys or (d_model // n_heads)
        # print("--------AttentionLayer-d_keys1----------", d_keys)
        d_values = d_values or (d_model // n_heads)
        # print("--------AttentionLayer-d_values----------", d_values)
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        # print("----------------self.query_projection(queries)------------",self.query_projection(queries).shape)
        # print("----------------self.key_projection(keys)------------",self.key_projection(keys).shape)
        # print("----------------self.value_projection(values)------------",self.value_projection(values).shape)

        queries = self.query_projection(queries).view(B, L, H, -1)
        keys = self.key_projection(keys).view(B, S, H, -1)
        values = self.value_projection(values).view(B, S, H, -1)
        # print("----------------queries-xxx-----------",queries.shape)
        # print("----------------keys--xxx----------",keys.shape)
        # print("----------------values--xxx----------",values.shape)
        # print("----------------attn_mask--xxx----------",attn_mask)
        # print("----------------tau--xxx----------", tau)
        # print("----------------delta--xxx----------", delta)
        ...
        # print("----------------out--xxx00----------",out.shape)
        # print("----------------attn--xxx----------",attn)
        out = out.view(B, L, -1)
        # print("----------------out--xxx01----------", out.shape)
        # print("----------------out--self.out_projection(out)----------", self.out_projection(out).shape)
...
if __name__=='__main__':
    ...
    #
    # net = LoopTransformer_itr05(num_nodes=307,supports=supports)
    # x = net(input)
