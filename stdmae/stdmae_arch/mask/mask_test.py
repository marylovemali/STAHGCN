...
class Mask(nn.Module):
    ...
    def encoding(self, long_term_history, mask=True):
        ...
        print("-----------mask值-----------------",mask)
        print("-----------self.spatial-----------------", self.spatial)
        if mask:
            if self.spatial:
            ...
            print("-------------long_term_history.shape-------------",long_term_history.shape)
            # patchify and embed input
            patches = self.patch_embedding(long_term_history)     # B, N, d, P
            print("-------------patches.shape-------------", patches.shape)
            patches = patches.transpose(-1, -2)         # B, N, P, d
            print("-------------patches.transpose(-1, -2) -------------", patches.shape)
            # positional embedding
            patches,self.pos_mat = self.positional_encoding(patches)# B, N, P, d
            print("-------------self.positional_encoding(patches) -------------", patches.shape)
            ...
            print("------------self.encoder(encoder_input) -------------", hidden_states_unmasked.shape)
            if self.spatial:
            ...
            print("-------------self.encoder_normxxx--------------",self.encoder_norm(hidden_states_unmasked).shape)
            print("------------self.encoder_norm(hidden_states_unmasked) -------------", hidden_states_unmasked.shape)
            print("------------unmasked_token_index -------------", unmasked_token_index)
            print("------------masked_token_index -------------", masked_token_index)
...
if __name__ == '__main__':
    ...
    # -----------------输入入口2xxxxx - --long_history_data - --------- torch.Size([1, 864, 307, 1])
    # -----------------输入入口2 - --hidden_states_t - ------- torch.Size([1, 307, 72, 96])
    # -----------------输入入口2 - --hidden_states_s - ------- torch.Size([1, 307, 72, 96])
    # "patch_size": 12,
    # "in_channel": 1,
    # "embed_dim": 96,
    # "num_heads": 4,
