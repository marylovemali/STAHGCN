import math
from torch import nn
from torch.nn import TransformerEncoder, TransformerEncoderLayer


class TransformerLayers(nn.Module):
    def __init__(self, hidden_dim, nlayers, mlp_ratio, num_heads=4, dropout=0.1):
        super().__init__()
        self.d_model = hidden_dim
        encoder_layers = TransformerEncoderLayer(hidden_dim, num_heads, hidden_dim*mlp_ratio, dropout)
        print("-----------xxxx00--解决这里-----------")
        print(encoder_layers)
        print("-------------------------------------------------------")
        self.transformer_encoder = TransformerEncoder(encoder_layers, nlayers)
        print("-----------xxxx01-------------")
        print(self.transformer_encoder)
        print("-------------------------------------------------")

    def forward(self, src):
        B, N, L, D = src.shape
        print("-----------xx--TransformerLayers--------",src.shape)
        print("-----------self.d_model--------",self.d_model)
        print("-----------math.sqrt(self.d_model)--------", math.sqrt(self.d_model))
        src = src * math.sqrt(self.d_model)
        print("-----------src-xxxx-------", src.shape)
        src=src.contiguous()
        print("-----------src.contiguous()-------", src.shape)

        src = src.view(B*N, L, D)
        print("-----------src.view(B*N, L, D)-------", src.shape)
        src = src.transpose(0, 1)
        print("----------src.transpose(0, 1)-------", src.shape)
        output = self.transformer_encoder(src, mask=None)
        print("----------self.transformer_encoder(src, mask=None)-------", output.shape)
        output = output.transpose(0, 1).view(B, N, L, D)
        print("----------output.transpose(0, 1).view(B, N, L, D)-------", output.shape)
        return output
